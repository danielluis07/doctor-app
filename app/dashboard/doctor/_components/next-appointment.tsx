...
import { useGetDoctorNextAppointment } from "@/queries/doctors/use-get-next-appointment";

export const NextAppointment = ({ id }: { id: string | undefined }) => {
  const { data, isLoading } = useGetDoctorNextAppointment(id);
  ...
  return (
    <div className="col-span-1 lg:col-span-2 bg-white p-4 md:p-6 rounded-lg shadow">
      ...
      <Link href={`/dashboard/doctor/appointments/${data?.appointment.id}`}>
